...
void exynos5_usb_phy20_init(void)
{
    ...
    // Likely not needed on Exynos 5410
    ...
    // Likely not needed on Exynos 5410
	//DEBUG ((EFI_D_ERROR, "ETC6PUD: 0x%X\n", MmioRead32(ETC6PUD)));
	//MmioWrite32(ETC6PUD, (MmioRead32(ETC6PUD) & ~(0x3 << 14)) | (0x3 << 14));
    //DEBUG ((EFI_D_ERROR, "ETC6PUD (enabled): 0x%X\n", MmioRead32(ETC6PUD)));

    // Change MUX to 0x1 (host mode), 0x0 is device mode
    MmioWrite32(EXYNOS5_USB_CFG, 0x1);

	MmioWrite32(EXYNOS5_USBHOST_PHY_CONTROL, 0x1);
    ...
    otgphy_sys = MmioRead32(PHY_OTG_SYS);
	...
	otgphy_sys |= (0x5 << OTG_SYS_CLKSEL_SHIFT); // TODO: explain
	...
	MmioWrite32(PHY_OTG_SYS, otgphy_sys);
	MicroSecondDelay(10);
    MicroSecondDelay(10);
	otgphy_sys &= ~(OTG_SYS_PHY0_SW_RST |
			OTG_SYS_LINK_SW_RST_UOTG |
	...
	ohcictrl = MmioRead32(PHY_HOST_OHCICTRL);
	ohcictrl |= OHCICTRL_SUSPLGCY;
	MmioWrite32(PHY_HOST_OHCICTRL, ohcictrl);
...
static int exynos5_usb_host_phy30_is_on(void)
{
	return MmioRead32(EXYNOS5_USBDEV_PHY_CONTROL) ? 1 : (MmioRead32(EXYNOS5_USBDEV1_PHY_CONTROL) ? 1 : 0);
}
...
void exynos5_usb_phy30_init(void)
{
	...
	MmioWrite32(EXYNOS5_USBDEV_PHY_CONTROL, 0x1);
    // Likely not needed on Exynos 5410
    //MmioWrite32(EXYNOS5_USBDEV1_PHY_CONTROL, 0x1);
